...
This file is used to generate a dataset containing only IFE granular and spinous.
...
```{r see_ifegs_markers, fig.width = 12, fig.height = 3}
ifegs_markers = c("SPINK5", "KRT1", "LY6D")
ifegs_cell_type = c("IFE granular spinous")
color_markers[!(names(color_markers) %in% ifegs_cell_type)] = "gray92"

# Feature Plot
plot_list = lapply(ifegs_markers, FUN = function(one_gene) {
...
plot_list[[length(plot_list) + 1]] = Seurat::DimPlot(sobj, group.by = "cell_type",
  ...
  ggplot2::labs(title = "Cell annotation",
                subtitle = paste0(sum(sobj$cell_type %in% ifegs_cell_type),
                                  " cells")) +
...
plot_list[[length(plot_list) + 1]] = Seurat::DimPlot(sobj,
  ...
  ggplot2::scale_color_manual(values = c(unname(unlist(color_markers[ifegs_cell_type])),
                                         rep("gray92", length(color_markers) - length(ifegs_cell_type))),
                              breaks = c(ifegs_cell_type, setdiff(names(color_markers), ifegs_cell_type))) +
  ggplot2::labs(title = "Cluster annotation",
                subtitle = paste0(sum(sobj$cluster_type %in% ifegs_cell_type),
                                  " cells")) +
  Seurat::NoAxes() + Seurat::NoLegend() +
  ggplot2::theme(aspect.ratio = 1,
                 plot.title = element_text(hjust = 0.5),
...
We extract cells based on the cell type annotation and cluster annotation. We also add the cluster 24 which corresponds to proliferative cells.
...
sobj$of_interest = (sobj$cluster_type %in% ifegs_cell_type) |
  (sobj$cell_type %in% ifegs_cell_type) |
  (sobj$seurat_clusters %in% "24")

sobj = subset(sobj, of_interest == TRUE)
sobj
```
...
cluster_markers = c(ifegs_markers,
                    "KRT23", "ALDH1A1", "AR", "FGFR3", "SPRR1B",
                    "IL11RA", "TK1", "FAM111B", "CHI3L2", "TOP2A",
                    # QC metrics
