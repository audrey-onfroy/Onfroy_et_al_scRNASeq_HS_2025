...
plot_list = lapply(new_columns, FUN = function(one_gs) {
  gs_name = stringr::str_remove(one_gs, pattern = "score_")
  
  p = Seurat::VlnPlot(sobj, features = one_gs,
                      group.by = "cluster_type",
                      split.by = "sample_type") +
    ggplot2::scale_fill_manual(values = c("#C55F40", "#2C78E6"),
                                breaks = c("HS", "HD")) +
    ggplot2::labs(title = gs_name) +
