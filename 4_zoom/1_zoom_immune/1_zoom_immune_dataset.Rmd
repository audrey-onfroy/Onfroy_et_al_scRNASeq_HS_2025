...
This file is used to generate a dataset containing only immune cells.
...
```{r see_immune_markers, fig.width = 12, fig.height = 3}
immune_markers = c("PTPRC", "AIF1", "CD3E")
immune_cell_type = c("CD4 T cells", "CD8 T cells",
                     "Langerhans cells", "macrophages", "B cells")
color_markers[!(names(color_markers) %in% immune_cell_type)] = "gray92"

# Feature Plot
plot_list = lapply(immune_markers, FUN = function(one_gene) {
...
plot_list[[length(plot_list) + 1]] = Seurat::DimPlot(sobj, group.by = "cell_type",
  ...
  ggplot2::labs(title = "Cell annotation",
                subtitle = paste0(sum(sobj$cell_type %in% immune_cell_type),
                                  " cells")) +
...
plot_list[[length(plot_list) + 1]] = Seurat::DimPlot(sobj,
  ...
  ggplot2::scale_color_manual(values = c(unname(unlist(color_markers[immune_cell_type])),
                                         rep("gray92", length(color_markers) - length(immune_cell_type))),
                              breaks = c(immune_cell_type, setdiff(names(color_markers), immune_cell_type))) +
  ggplot2::labs(title = "Cluster annotation",
                subtitle = paste0(sum(sobj$cluster_type %in% immune_cell_type),
                                  " cells")) +
  Seurat::NoAxes() + Seurat::NoLegend() +
  ggplot2::theme(aspect.ratio = 1,
                 plot.title = element_text(hjust = 0.5),
...
sobj = subset(sobj, cluster_type %in% immune_cell_type)
sobj
```
...
cluster_markers = c("PTPRC",
                    # T cells
                    "CD3E", "CD8A", "CD4", "FOXP3", 
                    "TNFRSF18","KLRD1", "CCL5", "GZMK",
                    # B cells
                    "CD79B",
                    # Macrophages
                    "AIF1", "TREM2", "S100A9", "CLEC10A", "CD209",
                    # Langerhans cells
                    "CD207", "CD1A",
                    # QC metrics
                    "TOP2A", "MCM5",
...
```{r plot_genes_oi, fig.width = 12, fig.height = 12}
