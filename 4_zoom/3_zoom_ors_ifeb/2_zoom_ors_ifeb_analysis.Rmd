...
aquarius::plot_barplot(df = table(sobj$sample_identifier,
  ...
  ggplot2::scale_fill_manual(values = c(colorRampPalette(c("chartreuse1", "chartreuse4"))(table(sort(cell_type_clusters))["ORS"]),
                                        colorRampPalette(c("cadetblue1", "cadetblue4"))(table(sort(cell_type_clusters))["IFE basal"])),
...
- cluster 5 vs rest of IFE basal, because this cluster is specific to HS compared to HD
...
## Cluster 5 within IFE basal

```{r group_name_cluster_5}
group_name = "cluster5_vs_IFE basal"
```
...
ggplot2::ggplot(mark, aes(x = pct.1, y = pct.2, col = avg_logFC)) +
  ...
  ggrepel::geom_label_repel(data = mark_signif, max.overlaps = Inf,
                            ...
                            col = "black", fill = NA, size = 3, label.size = NA) +
  ggplot2::labs(title = "Differentially expressed genes",
                subtitle = "between cluster 5 and other IFE basal") +
  ggplot2::scale_color_gradient2(low = aquarius::color_cnv[1],
                                 mid = aquarius::color_cnv[2],
                                 high = aquarius::color_cnv[3],
                                 midpoint = 0) +
...
enrichr_results = aquarius::run_enrichr(gene_names = genes_of_interest,
                                        ...
                                        gene_sets = gene_sets[, c("gs_name", "ensembl_gene")],
                                        make_plot = TRUE,
                                        plot_title = "Up-regulated in cluster 5 compared to other IFE basal")
...
enrichr_results = aquarius::run_enrichr(gene_names = genes_of_interest,
                                        ...
                                        gene_sets = gene_sets[, c("gs_name", "ensembl_gene")],
                                        make_plot = TRUE,
                                        plot_title = "Down-regulated in cluster 5 compared to other IFE basal")
...
features_oi = c(mark_to_label$gene_name,
                ...
                rownames(list_results$IFE basal_HS_vs_HD$mark)) %>%
  unique()
