...
This file is used to generate a dataset containing only hair follicle stem cells (matrixs).
...
```{r see_matrix_markers, fig.width = 12, fig.height = 3}
matrix_markers = c("MSX2", "BIRC5", "MCM5")
matrix_cell_type = c("cuticle", "cortex", "medulla", "IRS")
color_markers[!(names(color_markers) %in% matrix_cell_type)] = "gray92"

# Feature Plot
plot_list = lapply(matrix_markers, FUN = function(one_gene) {
...
plot_list[[length(plot_list) + 1]] = Seurat::DimPlot(sobj, group.by = "cell_type",
  ...
  ggplot2::labs(title = "Cell annotation",
                subtitle = paste0(sum(sobj$cell_type %in% matrix_cell_type),
                                  " cells")) +
...
plot_list[[length(plot_list) + 1]] = Seurat::DimPlot(sobj,
  ...
  ggplot2::scale_color_manual(values = c(unname(unlist(color_markers[matrix_cell_type])),
                                         rep("gray92", length(color_markers) - length(matrix_cell_type))),
                              breaks = c(matrix_cell_type, setdiff(names(color_markers), matrix_cell_type))) +
  ggplot2::labs(title = "Cluster annotation",
                subtitle = paste0(sum(sobj$cluster_type %in% matrix_cell_type),
                                  " cells")) +
  Seurat::NoAxes() + Seurat::NoLegend() +
  ggplot2::theme(aspect.ratio = 1,
                 plot.title = element_text(hjust = 0.5),
...
sobj = subset(sobj, cluster_type %in% matrix_cell_type)
sobj
```
...
We make a violin plot to visualize the distribution of the number of genes detected :

```{r vln_nfeature, fig.width = 7, fig.height = 4}
Seurat::VlnPlot(sobj, features = "nFeature_RNA") +
  Seurat::NoLegend()
```

There is one bad cluster. We remove cells with less than 2000 genes detected :

```{r filter_nfeature}
sobj = subset(sobj, nFeature_RNA > 2000)
sobj
```
...
```{r see_umap_tsne_cell_type, fig.width = 8, fig.height = 4, class.source = "fold-hide"}
...
```{r annotated_heatmap, fig.width = 12, fig.height = 7}
cluster_markers = c("MSX2", "KRT35", "PRR9", "KRT31", "CRYAB", "SOX21",
                    "KRT32", "KRT82", "CYP26B1", "FLRT3", "KRT71", "KRT73",
                    "COL17A1", "ALDH1A3", "PRDM1", "MMP1", "KRT75", "MCM3")
