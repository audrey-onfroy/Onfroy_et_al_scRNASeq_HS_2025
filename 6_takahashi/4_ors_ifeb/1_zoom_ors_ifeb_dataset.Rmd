...
```{r load_atlas}
sobj = readRDS(paste0(out_dir, "/../3_combined/takahashi_sobj.rds"))
sobj
```
...
```{r see_ors_ifeb_markers, fig.width = 12, fig.height = 3}
...
Due to clustering that badly separate ORS from IFE granular/spinous, there is a mis-selection of cells of interest : false positive. However, annotation smoothing adds a lot of (all) false negative and clean the annotation. We extract cells of interest based on clustering. Then, we generate a new cluster to remove false positive.
...
plot_list = lapply(c("nFeature_RNA", ors_ifeb_markers,
                     c("SPINK5", "AZGP1", "PTPRC", "MSX2")), FUN = function(one_gene) {
...
We select clusters based on SPINK5 and AZGP1 (IFE granular/spinous) expression :

```{r select_clusters, fig.width = 10, fig.height = 5}
mean_score_thresh = 1.3

sobj$genes_expr = Seurat::FetchData(sobj, c("SPINK5", "AZGP1")) %>%
  rowSums()
score_by_clusters = sobj@meta.data %>%
  dplyr::select(seurat_clusters, genes_expr) %>%
  dplyr::group_by(seurat_clusters) %>%
  dplyr::summarise(avg_score = mean(genes_expr)) %>%
  as.data.frame()
...
ggplot2::ggplot(score_by_clusters, aes(x = seurat_clusters, y = avg_score)) +
  ...
  ggplot2::labs(x = "Cluster ID", y = "Mean SPINK5 + AZGP1 expression",
