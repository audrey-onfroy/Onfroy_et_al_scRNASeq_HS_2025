...
```{r plot_cell_type, fig.width = 12, fig.height = 16}
plot_list = lapply(sobj_list, FUN = function(one_sobj) {
...
We generate a UMAP and a UMAP with `r ndims` principal components :
...
```{r see_umap_umap, fig.width = 8, fig.height = 4, class.source = "fold-hide"}
...
```{r harmony_umap_umap, time_it = TRUE}
sobj = Seurat::RunUMAP(sobj, 
                       seed.use = 1337L,
                       dims = 1:ndims,
                       reduction = "harmony",
...
```{r see_umap_umap_after, fig.width = 8, fig.height = 4, class.source = "fold-hide"}
tsne = Seurat::DimPlot(sobj, group.by = "project_name",
