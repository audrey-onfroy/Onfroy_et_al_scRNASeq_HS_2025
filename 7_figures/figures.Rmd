...
Barplot with number of HF-SCss and total number of cells :
...
aquarius::plot_barplot(df = quantif_to_plot,
  ...
  ggplot2::scale_fill_manual(values = c("gray90", color_markers[["HF-SCs"]]),
...
```{r fig_ors_ifeb_cluster_type_split, fig.width = 8, fig.height = 4}
plot_list = aquarius::plot_split_dimred(sobj_ors_ifeb, reduction = name2D,
...
table(sobj_ors_ifeb$seurat_clusters,
      sobj_ors_ifeb$sample_identifier)

table(sobj_ors_ifeb$cluster_type,
      sobj_ors_ifeb$sample_identifier)
```
...
sobj_ors_ifeb$cluster_type_sep5 = ifelse(sobj_ors_ifeb$seurat_clusters == 5,
                                       yes = "ORS_5",
                                       no = as.character(sobj_ors_ifeb$cluster_type)) %>%
  as.factor()

table(sobj_ors_ifeb$cluster_type_sep5,
      sobj_ors_ifeb$sample_type)
```


Barplot by cluster category :

```{r fig_ors_ifeb_barplot, fig.width = 5.5, fig.height = 4.5}
quantif = table(sobj_ors_ifeb$sample_identifier) %>%
  as.data.frame.table() %>%
  `colnames<-`(c("Sample", "nb_cells"))

quantif_to_plot = table(sobj_ors_ifeb$sample_identifier,
                        sobj_ors_ifeb$cluster_type_sep5) %>%
  as.data.frame.table() %>%
  `colnames<-`(c("Sample", "CellType", "Number")) %>%
...
aquarius::plot_barplot(df = quantif_to_plot,
  ...
  ggplot2::geom_label(data = quantif, inherit.aes = FALSE,
                      aes(x = .data$Sample, y = 1.05, label = .data$nb_cells),
                      label.size = 0, size = 5) +
  ggplot2::scale_fill_manual(values = unlist(color_markers[levels(sobj_ors_ifeb$cluster_type)]),
                             breaks = names(color_markers[levels(sobj_ors_ifeb$cluster_type)]),
                             name = "Cell Type") +
  ggplot2::scale_y_continuous(breaks = seq(0, 1, by = 0.25),
                              labels = paste0(seq(0, 100, by = 25), sep = " %"),
                              expand = ggplot2::expansion(add = c(0, 0.05))) +
...
```{r fig_ors_ifeb_barplot_cluster5, fig.width = 5.5, fig.height = 4.5}
...
```{r fig_ors_ifeb_ors_split, fig.width = 6, fig.height = 4}
...
```{r fig_ors_ifeb_heatmap_cluster5, fig.width = 12, fig.height = 19, class.source = "fold-hide"}
subsobj = subset(sobj_ors_ifeb, cluster_type == "ORS")
...
ht = Heatmap(as.matrix(mat_expr),
             ...
             bottom_annotation = ha_bottom,
             # Cell grouping
             column_split = subsobj$sample_type,
...
# HF-SCss to ORS, IFE basal and IFE granular/spinous
